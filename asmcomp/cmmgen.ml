...
let rec emit_constant symb cst cont =
  ...
  | Uconst_base(Const_string (s,_)) | Uconst_immstring s ->
...
let predef_exception i name =
  ...
  Cdata(Cglobal_symbol symname ::
        emit_constant (true,symname)
          (Uconst_block(Obj.object_tag,
                        [Uconst_base(Const_string (name, None));
                         Uconst_base(Const_int (-i-1))
                        ])) [])
