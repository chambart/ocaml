...
module Conv(P:Param2) = struct
  ...
  and conv_approx (env : env) : P.t flambda -> unit flambda * approx = function
    ...
    | Fconst (Fconst_base c as cst,_) ->
      Fconst(cst, ()),
      (match c with
       | Const_int i -> Value_id (new_descr (Value_int i) env)
       | Const_char c -> Value_id (new_descr (Value_int (Char.code c)) env)
       | Const_string _
       | Const_float _
       | Const_int32 _
       | Const_int64 _
  ...
  and conv_closure env functs fv =
      ...
      { closure_id = functs.ident;
        bound_var =
          IdentMap.fold (fun off_id (_,approx) map ->
            ...
            fv_ulam_approx OffsetMap.empty } in
    let value_closure =
    ...
    let conv_function id func =
      ...
      in
      { func with body = conv env func.body }
    in
    ...
    let used_fv = IdentMap.filter (fun id _ ->
        not (is_constant id) || IdentSet.mem id used_env_field ) fv_ulam in
