...
let build_package_cmx members cmxfile =
  ...
  let pack_units =
    List.fold_left
      (fun set info ->
         Flambda.IdentSet.add (Ident.create_persistent info.ui_name) set)
      Flambda.IdentSet.empty units in
  let units = List.map (fun info ->
      { info with
        ui_export_info =
          Flambdaexport.import_for_pack ~pack_units
            ~pack:(Compilenv.current_unit_id ()) info.ui_export_info })
      units in
  let ui = Compilenv.current_unit_infos() in
  let ui_export_info =
    List.fold_left (fun acc info -> Flambdaexport.merge acc info.ui_export_info)
      (Flambdaexport.import_for_pack ~pack_units
         ~pack:(Compilenv.current_unit_id ()) ui.ui_export_info)
      units in
  let pkg_infos =
    { ui_name = ui.ui_name;
