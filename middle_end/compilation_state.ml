  ...
  let report_error ppf error =
    ...
    | Wrong_for_pack_prefix { expected_prefix; found_prefix; filename; } ->
      Format.fprintf ppf "%s@ contains the description for a unit with \
          a `-for-pack' prefix of@ %a when %a was expected"
        filename
        print_prefix expected_prefix
        print_prefix found_prefix
    | Illegal_import_for_pack_prefix { desired_unit_name;
        for_pack_prefix_in_cmx; current_for_pack_prefix; filename; } ->
      (* CR mshinwell: fix this message so it's clear *)
      Format.fprintf ppf "%s@ contains the description for a symbol %a whose \
          compilation unit has a `-for-pack' prefix of@ %a; this cannot be \
          imported into a context where the `for-pack' prefix is@ %a"
        filename
        CU.Name.print desired_unit_name
...
let global_infos_table = CU.Name.Tbl.create 17
...
let find_or_load_unit_info_from_cmx desired_unit_name : find_cmx_result =
  let current_unit_name = CU.name (CU.get_current_exn ()) in
  if CU.Name.equal desired_unit_name current_unit_name then
    Current_unit
  else
    try
      Already_loaded (CU.Name.Tbl.find global_infos_table desired_unit_name)
    with Not_found ->
      ...
      let info_and_crc =
        if Env.is_imported_opaque desired_unit_name_as_string then None
          ...
          try
            let filename =
              Load_path.find_uncap (desired_unit_name_as_string ^ ".cmx")
            in
      ...
      | Some (info, crc, filename) ->
        ...
        CU.Name.Tbl.add global_infos_table desired_unit_name info;
...
let compilation_unit_for_global id : compilation_unit_or_predef =
  ...
  if Ident.is_predef id then begin
    ...
    | Just_loaded { info; filename; } ->
      ...
      in
      (* CR mshinwell: check error, we may be able to reuse an existing one *)
...
let cache_unit_info ui =
  CU.Name.Tbl.add global_infos_table (CU.name (UI.unit ui)) ui
...
module Flambda_only = struct
  ...
  let export_info_for_unit comp_unit =
    ...
    in
    match find_or_load_unit_info_from_cmx name_of_unit_containing_symbol with
